...
import {PooledExecute} from "src/PooledExecute.sol";
...
contract PooledExecuteTest is TestUtils {
  ...
  ExecuteTarget internal target = new ExecuteTarget();
  ...
  function test_doesNotExecuteIfNotEnoughEth() external {
    ...
    pool.join{value: 1}();
    assertEq(
      uint8(pool.executeResult()),
      uint8(PooledExecute.ExecuteResult.NotExecuted)
    );
  }
  ...
  function test_cannotReenterJoin() external {
    ...
    vm.expectEmit(false, false, false, true);
    emit ExecuteFailed(
      abi.encodeWithSignature("Error(string)", "already executed")
    );
    pool.join{value: 1}();
  }
  ...
  function test_cannotReenterWithdraw() external {
    ...
    vm.expectEmit(false, false, false, true);
    emit ExecuteFailed(
      abi.encodeWithSignature("Error(string)", "execution hasn't failed")
    );
    pool.join{value: 1}();
  }
