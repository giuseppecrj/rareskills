...
contract TokenWithStaking is ReentrancyGuard {
  ...
  function withdraw(uint256 _tokenId) external nonReentrant {
    ...
    require(stakers[staker].amountStaked > 0, "You have no tokens staked");

    // Wallet must own the token they are trying to withdraw
    require(stakerAddress[_tokenId] == staker, "You don't own this token!");

    // Update the rewards for this user, as the amount of rewards decreases with less tokens.
    uint256 rewards = calculateRewards(staker);
    stakers[staker].unclaimedRewards += rewards;
